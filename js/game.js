...
(function () {
  ...
  // 难度数据
  let beatsByDifficulty = [];  // [{ label, beats, color }, ...]
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      selectedDifficulty = beatsByDifficulty.length - 1; // 默认最难
  ...
  function buildDifficulties(rawOnsetsWithFlux, fullBeats) {
    ...
    // 按 DIFF_META 顺序（EASY→NORMAL→HARD）取末尾 tierCount 个
  ...
  function render(songTime) {
    ...
    // 里程碑全屏闪光（在 tracks 下方）
    const flashAge = (performance.now() - milestoneFlash.until * 1000 + milestoneFlash.duration * 1000) / 1000;
  ...
  function drawTracks() {
    for (let t = 0; t < TRACK_COUNT; t++) {
      ...
      ctx.fillText(AudioAnalyzer.TRACKS[t].label, bx, by);
    }
  }
